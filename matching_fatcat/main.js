...
const mappingPath = "resources/fatcat_glutton_mapping.json";
            ...
            function deleteIndex(existence, callback) {
                ...
                if (existence) {
                    client.indices.delete({
                        ...
                        console.log('Index fatcat_glutton have been deleted', resp);
...
/**
 * This function removes some non-used stuff from a fatcat release entry,
...
function index(options) {
    var readStream = fs.createReadStream(options.dump)
        ...
        .pipe(es.map(function (data, cb) {
            ...
            if (data.pages) {
                var pagePieces = data.pages.split(/,|-| /g);
            ...
            if (data.container) {
                obj.journal = data.container.name;
        ...
        if (batch.length > 0) {
            ...
            client.bulk({
                ...
                if (err) {
                    ...
                    console.log('Completed fatcat indexing.');
                    next();
                }
            });
        } else {
    ...
    if (options.dump) {
        fs.lstat(options.dump, (err, stats) => {
            ...
            if (stats.isDirectory())
                console.log("fatcat dump path must be a file, not a directory");
            if (!stats.isFile())
                console.log("fatcat dump path must be a valid file");
        });
    }
